...
class FFChatInfo {
  ...
  final List<UserProfileRecord>? groupMembers;

  UserProfileRecord get currentUser => groupMembers!
      .where((user) => user.reference == currentUserReference)
      .first;
  Map<String, UserProfileRecord> get otherUsers => Map.fromEntries(
        groupMembers!
            .where((user) => user.reference != currentUserReference)
            .map((user) => MapEntry(user.reference.id, user)),
      );
  List<UserProfileRecord> get otherUsersList => otherUsers.values.toList();
...
class FFChatManager {
  ...
  Stream<FFChatInfo> getChatInfo({
    UserProfileRecord? otherUserRecord,
    DocumentReference? chatReference,
    ...
    return chatStream.asyncMap((chat) async {
      ...
      final groupMembers = await Future.wait(
        userRefs.map((ref) => UserProfileRecord.getDocument(ref).first),
      );
